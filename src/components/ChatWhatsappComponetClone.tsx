...
const ImageMessage: React.FC<{ src: string; isOutgoing: boolean }> = ({ src, isOutgoing }) => (
  <img src={src} alt="Shared image" className={`max-w-xs lg:max-w-sm rounded-lg ${isOutgoing ? 'ml-auto' : 'mr-auto'}`} />
)

const VideoMessage: React.FC<{ src: string; isOutgoing: boolean }> = ({ src, isOutgoing }) => (
  <video controls className={`max-w-xs lg:max-w-sm rounded-lg ${isOutgoing ? 'ml-auto' : 'mr-auto'}`}>
    <source src={src} type="video/mp4" />
    Your browser does not support the video tag.
  </video>
)

const VoiceMessage: React.FC<{ src: string; duration: number; isOutgoing: boolean }> = ({ src, duration, isOutgoing }) => {
  ...
  return (
    <div className={`flex items-center space-x-2 ${isOutgoing ? 'justify-end' : 'justify-start'}`}>
      <button
        onClick={togglePlay}
...
const DocumentMessage: React.FC<{ fileName: string; fileSize: string; isOutgoing: boolean }> = ({ fileName, fileSize, isOutgoing }) => (
  <div className={`flex items-center space-x-2 ${isOutgoing ? 'justify-end' : 'justify-start'}`}>
    <File size={24} />
    <div>
...
export default function EnhancedWhatsAppChat() {
